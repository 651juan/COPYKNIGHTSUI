    ...
    d3.json("http://localhost:8080/article", function(error, result) {
        ...
        for (var article in articles) {
            ...
            for (var reference in articles[article]['references']) {
                var tmpLink = {};
                allArticlesReferences[articles[article]['references'][reference].trim()] = 1;

                tmpLink['source'] = articles[article]['name'].trim();
                tmpLink['target'] = articles[article]['references'][reference].trim();
                tmpLink['value'] = 1;
        ...
        for (var node in allArticlesReferences) {
            var currNode = {};
            currNode['id'] = node;
            currNode['group'] = allArticlesReferences[node];
            tmpNodes.push(currNode);
        }

        graph.nodes = tmpNodes;
        graph.links = tmpLinks;
