...
//Changed here
//check that l\ocal var is set
if(localStorage.getItem("currentPath") === null) {
	localStorage.setItem("currentPath", mainMenuJson);
}
//End Change

d3.json(localStorage.getItem("currentPath"), function(error, root) {
	//Changed Here
	localStorage.setItem("currentPath", mainMenuJson); //Reset Path so refresh takes user back to main menu
	//End Change
	console.log(error);
	...
	//var nSubBubble = Math.floor(root.children[iB].children.length/2.0);   
		...
		childBubbles.append("circle")
			...
			.on("click", function(d,i) {
				//Changed Here
				localStorage.setItem("currentPath", d.address);
				location.reload();
